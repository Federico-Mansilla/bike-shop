...
RETURN_MENU() {
  ...
  then
    ...
    CUSTOMER_RENTALS=$($PSQL "SELECT bike_id, type, size FROM BIKES INNER JOIN rentals USING(bike_id) INNER JOIN customers USING(customer_id) WHERE date_returned IS NULL AND phone='$PHONE_NUMBER' ORDER BY bike_id")
    ...
    then
      ...
      then
       ...
       MAIN_MENU "That is not a valid bike number."
        ...
        WHERE phone='$PHONE_NUMBER' AND bike_id=$BIKE_ID_TO_RETURN  AND date_returned IS NULL")
        # if input not rented 
        if [[ -z $RENTAL_ID ]]
        then
          # send to main menu
